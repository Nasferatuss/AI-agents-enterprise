...
| **Redis / Qdrant unauthenticated by default** | ✅ both now support auth, **off by default**: Redis honors `WB_REDIS_PASSWORD`, Qdrant `WB_QDRANT_API_KEY` (compose passes it as `QDRANT__SERVICE__API_KEY`; the client sends it). With `WB_JOB_BACKEND=redis` the job queue lives in Redis, so set `WB_REDIS_PASSWORD` and keep both stores on a private network before exposing the stack. Defense in depth: `Job` fields are pattern-validated and `dispatch` only runs *registered* kinds, so an injected job can't invent a handler. |
| Rate limiter client identity | ✅ uses the socket peer by default; set `WB_TRUST_PROXY_HEADERS=true` **only behind a trusted proxy** to bucket by `X-Forwarded-For` (the header is client-spoofable otherwise). |
| Hung `running` runs | ✅ a heartbeat (the handler bumps `updated_at`) + a periodic sweeper fail runs idle past `WB_RUN_STUCK_TTL_S`; a crash before terminal state is recovered by the startup reconciler. At-least-once delivery + an idempotent handler (terminal runs are not re-executed) gives effectively-once for finished work — not true exactly-once. |
